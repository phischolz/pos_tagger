...
package fasttag.src.com.knowledgebooks.nlp.fasttag;
...
public class FastTag {
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			...
			List<String> words = fasttag.src.com.knowledgebooks.nlp.util.Tokenizer.wordsToList("The ball rolled down the street.");
			List<String> tags = tag(words);
			for (int i = 0; i < words.size(); i++)
				System.out.println(words.get(i) + "/" + tags.get(i));
		} else {
			List<String> words = fasttag.src.com.knowledgebooks.nlp.util.Tokenizer.wordsToList(args[0]);
			List<String> tags = tag(words);
			for (int i = 0; i < words.size(); i++)
				System.out.println(words.get(i) + "/" + tags.get(i));
		}
	...
	private static Map<String, String[]> buildLexicon() {
		...
		try {
			...
			if (ins == null) {
				ins = new FileInputStream("fasttag/lexicon.txt");
			}
